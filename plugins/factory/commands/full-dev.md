---
description: "End-to-end feature development pipeline with design validation, parallel TDD, multi-persona review, and knowledge capture."
---

# /factory:full-dev - End-to-End Feature Development Pipeline

Strictly sequential phases. Each phase must satisfy Exit before proceeding.

**Feature**: the current user request in the chat
...
6. **Codex-native orchestration** - Use `spawn_agent` and plugin-local assets, not legacy tools

---

## Pre-flight

1. If the feature request is empty, ask the user for feature requirements and stop
2. Confirm these commands are available: `/factory:think`, `/factory:plan`, `/factory:hunt`, `/factory:build`, `/factory:check`, `/factory:compound`, `/factory:ship`
3. Any command unavailable: list missing items and stop
...
**Action:** Run `/factory:think` on the feature request, using a read-only research agent when the problem space is broad.

Follow the think skill's guidance to:
- Check `docs/solutions/` and `.factory/learnings.jsonl` for prior art
- Explore requirements with the user
...
**Action:** Run `/factory:plan` with the approved direction.
...
git status --short
git add -u
git commit -m "Pre-development checkpoint" --allow-empty
$MAIN_BRANCH = git branch --show-current
Write-Output "Main branch: $MAIN_BRANCH"
git worktree prune
```

Create the team by spawning agents. The main session remains coordinator only.
...
| Reviewer | Reviewer | 1 | Multi-persona code review via `spawn_agent` |
...
Reviewer MUST spawn a dedicated review agent and have it run the check workflow on the merged diff.
Scope: all changes since "Pre-development checkpoint".

**Reviewer Report:**
```
## Reviewer Report
- check workflow invoked: yes | no
...
**Coordinator validates:** `check workflow invoked` must be `yes`.
...
1. Invoke `/factory:ship` - present 4 options (merge/PR/keep/discard)
2. After ship action, invoke `/factory:compound` - capture:
...
| 2b | Main | /factory:hunt | On-demand root-cause debugging |
| 3.1 | Main | Setup + spawn agents | Team ready |
...
| `/factory:think` | 1 |
| `/factory:plan` | 2 |
| `/factory:hunt` | on demand for root-cause debugging |

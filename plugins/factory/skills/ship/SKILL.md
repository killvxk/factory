...
- If the change touches the Factory Codex plugin itself, confirm the plugin manifest, command surface, and memory/knowledge updates are all included in the merge
...
Before closing, confirm the merged result still passes the same checks that were green on the branch and that any plugin or docs changes are reflected in the final PR or commit.

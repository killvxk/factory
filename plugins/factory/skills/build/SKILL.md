...
This is not a suggestion. It is the rule.
...
- Use `spawn_agent` for parallel subagents and keep the main session as coordinator only
...
2. For each independent task, dispatch a subagent in its own worktree with `spawn_agent`:
